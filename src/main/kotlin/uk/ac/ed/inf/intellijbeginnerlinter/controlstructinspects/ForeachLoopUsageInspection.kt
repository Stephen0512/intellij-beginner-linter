...
/**
 * A customized inspection that checks if or how many of the "enhanced for loop" is used in the function.
 * The inspection can be configured to apply this inspection to all functions or only to specified functions.
 * Ihe number of "enhanced for loop" allowed in each function can also be specified by the user.
 *
 * @author Zhuohang (Stephen) Shen <s2023501@ed.ac.uk>
 */
class ForeachLoopUsageInspection : AbstractBaseJavaLocalInspectionTool() {
     ...
     * the new enhanced for loop usage inspection.
     *
     ...
     * @return A JavaElementVisitor that will inspect the usage of enhanced for loops in Java functions.
     */
    @NotNull
    override fun buildVisitor(@NotNull holder: ProblemsHolder, isOnTheFly: Boolean): PsiElementVisitor {
        return object : JavaElementVisitor() {

            // Declare the mutable map for counting the number of "enhanced for loop"s used in each function.
            private val methodForeachLoopCounts = mutableMapOf<PsiMethod, Int>()

            // Declare the mutable map for storing the number of "enhanced for loop"s allowed in each function.
            ...
            override fun visitForeachStatement(statement: PsiForeachStatement) {
                ...
                // Call the visitForeachStatement function from the super class to maintain the original behavior of it.
                ...
                // Find the java function which the enhanced for loop belongs to or return directly for no method.
                val method = PsiTreeUtil.getParentOfType(statement, PsiMethod::class.java) ?: return

                // Update the number of enhanced for loops used in the method.
                val count = methodForeachLoopCounts.getOrDefault(method, 0) + 1  // 0 as the initial values.
                methodForeachLoopCounts[method] = count
                ...
                // Find the number of enhanced for loops allowed in the method.
                ...
                if (count > allowance && allowance != -1) {
                    ...
                    if (allowance == 0) {
                        holder.registerProblem(
                            statement,
                            "Avoid using enhanced 'for' loops within Java methods '${method.name}'.",
                            ProblemHighlightType.WARNING
                        )
                    } else {  // Otherwise
                        holder.registerProblem(
                            statement,
                            "Too many enhanced 'for' loops within Java methods '${method.name}' (Allowed: ${allowance}; Used: ${count}).",
                            ProblemHighlightType.WARNING
                        )
                    }
                }
